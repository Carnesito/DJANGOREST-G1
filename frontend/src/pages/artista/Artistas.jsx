...
import { getArtistas, deleteArtista } from "../../services/artista/artistaService";
              ...
              <div className="flex flex-wrap items-center">
                <div className="relative w-full px-4 max-w-full flex-grow flex-1">
                  <h3 className="font-bold text-xl text-green-400" style={{ fontSize: '24px' }}>
                    <i className="fas fa-music mr-2"></i> Módulo de Artistas
                  </h3>
                </div>
                <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                  <Link
                    to="/dashboard/artistas/crear"
                ...
                <tbody>
                  {data.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-800 transition-colors">
                      ...
                      <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm whitespace-nowrap p-4 border-gray-700">
                        <Link
                          to={`/dashboard/artistas/editar/${item.id}`}
